...
class Tip {
  ...
  static Tip get instance => _tip ??= Tip._();

  Tip._();

  BuildContext context;

  static AlertTipsDialog _showingLoading;

  //等待的loading
  loading({barrierDismissible = true}) =>
  ...
  //----------toast-------------
//  final _toastKey = GlobalKey<_ToastState>();
//  OverlayEntry _overlayEntry;
//
//  toast(String msg, {duration: 2}) {
//    if (_overlayEntry == null) {
//      _overlayEntry = OverlayEntry(
//          builder: (BuildContext context) => Toast(_toastKey, msg));
//      Overlay.of(context).insert(_overlayEntry);
//    }
//    _toastKey.currentState.show(msg, duration: duration ?? 2);
//  }
  ...
  snack(
    ...
    var snackBar = SnackBar(
        content: Text(msg),
        duration: Duration(seconds: duration ?? label == null ? 2 : 6),
        action: label != null
        ...
        Container(
            ...
            child: CircularProgressIndicator(
              ...
              valueColor: ColorTween(
                      ...
                      end: Colors.deepPurpleAccent.withOpacity(0.54))
                  .animate(CurvedAnimation(
                      parent: _controller,
                      curve:
                          const Interval(0.1, 0.9, curve: Curves.easeInCubic))),
            )),
        Text('ADING', style: style),
      ],
    );
  }
}
...
class Toast extends StatefulWidget {
  ...
  Toast(Key key, this.message) : super(key: key);
...
class _ToastState extends State<Toast> {
  ...
  show(message, {int duration}) {
    setState(() {
      _message = message;
      if (_opacity == 1.0 && _future != null) _future = null;
      _duration = Duration(seconds: duration ?? 2);
      _opacity = 1.0;
...
class TipsDialog extends StatelessWidget {
  /// Creates a dialog.
  ///
  /// Typically used in conjunction with [showTipDialog].
  const TipsDialog({
    Key key,
    this.minWidth,
  ...
  /// The background color of the surface of this [TipsDialog].
  ///
  /// This sets the [Material.color] on this [TipsDialog]'s [Material].
  ///
  ...
  /// The z-coordinate of this [TipsDialog].
  ///
  ...
  /// Indicates whether the [TipsDialog.shape]'s default [RoundedRectangleBorder]
...
/// [AlertTipsDialog] tries to size itself using the intrinsic dimensions of its
...
/// using [TipsDialog] directly.)
///
/// For dialogs that offer the user a choice between several options, consider
/// using a [SimpleTipsDialog].
///
/// Typically passed as the child widget to [showTipDialog], which displays the
/// dialog.
///
...
/// See also:
///
///  * [SimpleTipsDialog], which handles the scrolling of the contents but has no [actions].
///  * [TipsDialog], on which [AlertTipsDialog] and [SimpleTipsDialog] are based.
///  * [CupertinoAlertDialog], an iOS-styled alert dialog.
///  * [showTipDialog], which actually displays the dialog and returns its result.
///  * <https://material.io/design/components/dialogs.html#alert-dialog>
class AlertTipsDialog extends StatelessWidget {
  ...
  /// Style for the text in the [title] of this [AlertTipsDialog].
  ///
  ...
  /// message. As noted in the [AlertTipsDialog] documentation, it's important
  ...
  /// Style for the text in the [content] of this [AlertTipsDialog].
  ///
...
/// An option used in a [SimpleTipsDialog].
///
...
/// default [SimpleTipsDialog.contentPadding] so that each option ends up 8 pixels
...
/// See also:
///
///  * [SimpleTipsDialog], for a dialog in which to use this widget.
///  * [showTipDialog], which actually displays the dialog and returns its result.
...
class SimpleDialogOption extends StatelessWidget {
  /// Creates an option for a [SimpleTipsDialog].
  ...
  ///
  /// When used in a [SimpleTipsDialog], this will typically call [Navigator.pop]
  /// with a value for [showTipDialog] to complete its future with.
...
/// options are represented as an enum. The [showTipDialog] method here returns
...
///  * [AlertTipsDialog], for dialogs that have a row of buttons below the body.
///  * [TipsDialog], on which [SimpleTipsDialog] and [AlertTipsDialog] are based.
///  * [showTipDialog], which actually displays the dialog and returns its result.
///  * <https://material.io/design/components/dialogs.html#simple-dialog>
class SimpleTipsDialog extends StatelessWidget {
  /// Creates a simple dialog.
  ///
  /// Typically used in conjunction with [showTipDialog].
  ///
  /// The [titlePadding] and [contentPadding] arguments must not be null.
  const SimpleTipsDialog({
    Key key,
    this.title,
  ...
  /// Style for the text in the [title] of this [SimpleTipsDialog].
  ///
  /// If null, [DialogTheme.titleTextStyle] is used, if that's null, defaults to
  /// [ThemeData.textTheme.headline6].
  final TextStyle titleTextStyle;
...
///
/// This function takes a `builder` which typically builds a [TipsDialog] widget.
...
/// See also:
///
///  * [AlertTipsDialog], for dialogs that have a row of buttons below a body.
///  * [SimpleTipsDialog], which handles the scrolling of the contents and does
///    not show buttons below its body.
///  * [TipsDialog], on which [SimpleTipsDialog] and [AlertTipsDialog] are based.
///  * [showCupertinoDialog], which displays an iOS-style dialog.
///  * [showGeneralDialog], which allows for customization of the dialog popup.
///  * <https://material.io/design/components/dialogs.html>
Future<T> showTipDialog<T>({
  @required
      BuildContext context,
...
extension Snack on String {
  snack({String label, int duration}) {
    Tip.instance.snack(this, label: label, duration: duration);
  }
}
