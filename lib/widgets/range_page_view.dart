...
/// A controller for [RangePageView].
///
/// A page controller lets you manipulate which page is visible in a [RangePageView].
/// In addition to being able to control the pixel offset of the content inside
/// the [RangePageView], a [RangePageController] also lets you control the offset in terms
/// of pages, which are increments of the viewport size.
///
/// See also:
///
///  * [RangePageView], which is the widget this object controls.
///
/// {@tool snippet}
///
/// This widget introduces a [MaterialApp], [Scaffold] and [RangePageView] with two pages
/// using the default constructor. Both pages contain an [ElevatedButton] allowing you
/// to animate the [RangePageView] using a [RangePageController].
///
/// ```dart
...
class RangePageController extends ScrollController {
  ...
  /// The page to show when first creating the [RangePageView].
  ...
  /// The current page displayed in the controlled [RangePageView].
  ///
  /// There are circumstances that this [RangePageController] can't know the current
  /// page. Reading [page] will throw an [AssertionError] in the following cases:
  ///
  /// 1. No [RangePageView] is currently using this [RangePageController]. Once a
  /// [RangePageView] starts using this [RangePageController], the new [page]
  /// position will be derived:
  ///
  ///   * First, based on the attached [RangePageView]'s [BuildContext] and the
  ///     position saved at that context's [PageStorage] if [keepPage] is true.
  ///   * Second, from the [RangePageController]'s [initialPage].
  ///
  /// 2. More than one [RangePageView] using the same [RangePageController].
  ///
  /// The [hasClients] property can be used to check if a [RangePageView] is attached
  ...
  /// Animates the controlled [RangePageView] from the current page to the given page.
  ///
  ...
  /// Changes which page is displayed in the controlled [RangePageView].
  ///
  ...
  /// Animates the controlled [RangePageView] to the next page.
  ///
  /// The animation lasts for the given duration and follows the given curve.
  /// The returned [Future] resolves when the animation completes.
  ///
  ...
  /// Animates the controlled [RangePageView] to the previous page.
  ///
  /// The animation lasts for the given duration and follows the given curve.
  /// The returned [Future] resolves when the animation completes.
  ///
...
class RangePageMetrics extends FixedScrollMetrics {
  ...
  /// The current page displayed in the [RangePageView].
  double get page {
...
class _PagePosition extends ScrollPositionWithSingleContext implements RangePageMetrics {
  _PagePosition({
...
/// Scroll physics used by a [RangePageView].
///
...
///    physics.
///  * [RangePageView.physics], which can override the physics used by a page view.
class RangePageScrollPhysics extends ScrollPhysics {
  /// Creates physics for a [RangePageView].
...
///
/// You can use a [RangePageController] to control which page is visible in the view.
/// In addition to being able to control the pixel offset of the content inside
/// the [RangePageView], a [RangePageController] also lets you control the offset in terms
/// of pages, which are increments of the viewport size.
///
/// The [RangePageController] can also be used to control the
/// [RangePageController.initialPage], which determines which page is shown when the
/// [RangePageView] is first constructed, and the [RangePageController.viewportFraction],
...
/// See also:
///
///  * [RangePageController], which controls which page is visible in the view.
...
class RangePageView extends StatefulWidget {
  ...
  /// [RangePageView] will participate in accessibility scrolling more like a
  /// [ListView], where implicit scroll actions will move to the next page
  /// rather than into the contents of the [RangePageView].
  /// {@endtemplate}
  RangePageView({
    Key key,
  ...
  ///
  /// Providing a non-null [itemCount] lets the [RangePageView] compute the maximum
  /// scroll extent.
  ///
  /// [itemBuilder] will be called only with indices greater than or equal to
  /// zero and less than [itemCount].
  ///
  /// [RangePageView.builder] by default does not support child reordering. If
  /// you are planning to change child order at a later time, consider using
  /// [RangePageView] or [RangePageView.custom].
  ///
  ...
  /// {@tool snippet}
  ///
  /// This [RangePageView] uses a custom [SliverChildBuilderDelegate] to support child
  /// reordering.
  ///
  /// ```dart
  /// class MyPageView extends StatefulWidget {
...
class _RangePageViewState extends State<RangePageView> {
    ...
    return NotificationListener<ScrollNotification>(
      onNotification: (ScrollNotification notification) {
        if (notification.depth == 0 && widget.onPageChanged != null && notification is ScrollUpdateNotification) {
          final RangePageMetrics metrics = notification.metrics as RangePageMetrics;
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder description) {
    ...
    description.add(DiagnosticsProperty<RangePageController>('controller', widget.controller, showName: false));
