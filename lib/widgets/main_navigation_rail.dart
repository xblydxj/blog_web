...
///
/// The appearance of all of the [MainNavigationRail]s within an app can be
...
///
/// This example shows a [MainNavigationRail] used within a Scaffold with 3
/// [MainNavigationRailDestination]s. The main content is separated by a divider
...
///    [Scaffold.body] slot.
///  * [MainNavigationRailDestination], which is used as a model to create tappable
...
class MainNavigationRail extends StatefulWidget {
  ...
  /// [MainNavigationRailDestination] values.
  ///
  ...
  /// true when when the [labelType] is null or [MainNavigationRailLabelType.none].
  ///
  ...
  const MainNavigationRail({
    ...
    this.minExtendedWidth,
        ...
        assert(!extended ||
            (labelType == null ||
                labelType == MainNavigationRailLabelType.none));

  /// Sets the color of the Container that holds all of the [MainNavigationRail]'s
  /// contents.
  ///
  ...
  /// Indicates that the [MainNavigationRail] should be in the extended state.
  ///
  ...
  /// must be set to [MainNavigationRailLabelType.none].
  ///
  ...
  ///
  /// The trailing widget is placed below the last [MainNavigationRailDestination].
  ...
  /// navigation rail.
  ///
  /// The value must be a list of two or more [MainNavigationRailDestination]
  /// values.
  final List<MainNavigationRailDestination> destinations;

  /// The index into [destinations] for the current selected
  /// [MainNavigationRailDestination].
  final int selectedIndex;

  /// Called when one of the [destinations] is selected.
  ///
  /// The stateful widget that creates the navigation rail needs to keep
  /// track of the index of the selected [MainNavigationRailDestination] and call
  ...
  ///
  /// The [MainNavigationRailDestination]s are grouped together with the [trailing]
  ...
  /// [MainNavigationRailLabelType.none].
  ///
  /// See also:
  ///
  ///   * [MainNavigationRailLabelType] for information on the meaning of different
  ///   types.
  final MainNavigationRailLabelType labelType;
  ...
  ///
  /// When a [MainNavigationRailDestination] is not selected,
  ...
  /// a copy of the [IconThemeData.fallback] with a custom [MainNavigationRail]
  ...
  /// Returns the animation that controls the [MainNavigationRail.extended] state.
  ///
  ...
  /// extended states of the [MainNavigationRail].
  ///
  /// An instance of `ExtendableFab` would be created for
  /// [MainNavigationRail.leading].
  ///
  /// ```dart
  /// import 'dart:ui';
  ///
  ...
  static Animation<double> extendedAnimation(BuildContext context) {
    return context
        .dependOnInheritedWidgetOfExactType<
            _MainExtendedNavigationRailAnimation>()
        .animation;
  }
...
class _MainNavigationRailState extends State<MainNavigationRail>
  ...
  @override
  void didUpdateWidget(MainNavigationRail oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    final MainNavigationRailLabelType labelType = widget.labelType ??
        navigationRailTheme.labelType ??
        MainNavigationRailLabelType.none;

    return _MainExtendedNavigationRailAnimation(
...
class _MainRailDestination extends StatelessWidget {
  _MainRailDestination({
  ...
  Widget build(BuildContext context) {
    ...
    Widget content;
    switch (labelType) {
      case MainNavigationRailLabelType.none:
        ...
        break;
      case MainNavigationRailLabelType.selected:
        ...
        break;
      case MainNavigationRailLabelType.all:
        content = Container(
          constraints: BoxConstraints(
            minWidth: minWidth,
            minHeight: minWidth,
...
/// Defines the behavior of the labels of a [MainNavigationRail].
///
/// See also:
///
///   * [MainNavigationRail]
enum MainNavigationRailLabelType {
  /// Only the [MainNavigationRailDestination]s are shown.
  none,

  /// Only the selected [MainNavigationRailDestination] will show its label.
  ///
  /// The label will animate in and out as new [MainNavigationRailDestination]s are
  /// selected.
  selected,

  /// All [MainNavigationRailDestination]s will show their label.
  all,
}

/// Defines a [MainNavigationRail] button that represents one "destination" view.
///
/// See also:
///
///  * [MainNavigationRail]
class MainNavigationRailDestination {
  /// Creates a destination that is used with [MainNavigationRail.destinations].
  ///
  /// [icon] and [label] must be non-null. When the [MainNavigationRail.labelType]
  /// is [MainNavigationRailLabelType.none], the label is still used for semantics,
  /// and may still be used if [MainNavigationRail.extended] is true.
  const MainNavigationRailDestination({
    @required this.icon,
    Widget selectedIcon,
    this.label,
  ...
  ///
  /// To make the [MainNavigationRail] more accessible, consider choosing an
  ...
  ///
  /// If this icon is not provided, the [MainNavigationRail] will display [icon] in
  /// either state. The size, color, and opacity of the
  /// [MainNavigationRail.selectedIconTheme] will still apply.
  ///
  /// See also:
  ///
  ///  * [MainNavigationRailDestination.icon], for a description of how to pair
  ///    icons.
  final Widget selectedIcon;

  /// The label for the destination.
  ///
  /// The label must be provided when used with the [MainNavigationRail]. When the
  /// [MainNavigationRail.labelType] is [MainNavigationRailLabelType.none], the label is
  /// still used for semantics, and may still be used if
  /// [MainNavigationRail.extended] is true.
  final Widget label;
}

class _MainExtendedNavigationRailAnimation extends InheritedWidget {
  const _MainExtendedNavigationRailAnimation({
    Key key,
  ...
  @override
  bool updateShouldNotify(_MainExtendedNavigationRailAnimation old) =>
