...
/// See also:
///
///  * [MainTabBar], which displays a row of tabs.
///  * [MainTabBarView], which displays a widget for the currently selected tab.
///  * [MainTabBar.indicator], which defines the appearance of the selected tab
///    indicator relative to the tab's bounds.
enum MainTabBarIndicatorSize {
  ...
  ///
  /// This value is used to align the tab's label, typically a [MainTab]
  /// widget's text or icon, with the selected tab indicator.
  label,
}

/// A material design [MainTabBar] tab.
///
...
///  * [MainTabBar], which displays a row of tabs.
///  * [MainTabBarView], which displays a widget for the currently selected tab.
///  * [TabController], which coordinates tab selection between a [MainTabBar] and a [MainTabBarView].
///  * <https://material.io/design/components/tabs.html>
class MainTab extends StatelessWidget {
  /// Creates a material design [MainTabBar] tab.
  ///
...
class _MainTabStyle extends AnimatedWidget {
  const _MainTabStyle({
    Key key,
...
typedef _MainLayoutCallback = void Function(
    List<double> xOffsets, TextDirection textDirection, double width);

class _MainTabLabelBarRenderer extends RenderFlex {
  _MainTabLabelBarRenderer({
  ...
  _MainLayoutCallback onPerformLayout;
...
class _MainTabLabelBar extends Flex {
  _MainTabLabelBar({
    Key key,
  ...
  final _MainLayoutCallback onPerformLayout;

  @override
  RenderFlex createRenderObject(BuildContext context) {
    return _MainTabLabelBarRenderer(
      direction: direction,
      mainAxisAlignment: mainAxisAlignment,
      mainAxisSize: mainAxisSize,
      crossAxisAlignment: crossAxisAlignment,
  ...
  void updateRenderObject(
      BuildContext context, _MainTabLabelBarRenderer renderObject) {
...
class _MainIndicatorPainter extends CustomPainter {
  _MainIndicatorPainter({
    @required this.controller,
    @required this.indicator,
    @required this.indicatorSize,
    @required this.tabKeys,
    _MainIndicatorPainter old,
  ...
  final MainTabBarIndicatorSize indicatorSize;
  ...
  Rect indicatorRect(Size tabBarSize, int tabIndex) {
    ...
    assert(tabIndex >= 0);
    assert(tabIndex <= maxTabIndex);
    double tabTop, tabRight;
    switch (_currentTextDirection) {
      case TextDirection.rtl:
        tabTop = _currentTabOffsets[tabIndex + 1];
    ...
    if (indicatorSize == MainTabBarIndicatorSize.label) {
      final double tabHeight = tabKeys[tabIndex].currentContext.size.height;
      final double delta = ((tabRight - tabTop) - tabHeight) / 2.0;
      tabTop += delta;
      tabRight -= delta;
    }
//    return Rect.fromCenter(width: tabBarSize.width, height: tabBarSize.height);
    return Rect.fromLTWH(tabTop, 0.0, 0, 0);
  }
  ...
  @override
  bool shouldRepaint(_MainIndicatorPainter old) {
    return _needsPaint ||
...
class _MainChangeAnimation extends Animation<double>
    with AnimationWithParentMixin<double> {
  _MainChangeAnimation(this.controller);
...
class _MainDragAnimation extends Animation<double>
    with AnimationWithParentMixin<double> {
  _MainDragAnimation(this.controller, this.index);
...
class _MainTabBarScrollPosition extends ScrollPositionWithSingleContext {
  _MainTabBarScrollPosition({
...
class _MainTabBarScrollController extends ScrollController {
  _MainTabBarScrollController(this.tabBar);
  ...
  ScrollPosition createScrollPosition(ScrollPhysics physics,
    ...
    return _MainTabBarScrollPosition(
      physics: physics,
      context: context,
      oldPosition: oldPosition,
      tabBar: tabBar,
...
/// See also:
///
///  * [MainTabBarView], which displays page views that correspond to each tab.
class MainTabBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  /// Typically a list of two or more [MainTab] widgets.
  ///
  /// The length of this list must match the [controller]'s [TabController.length]
  /// and the length of the [MainTabBarView.children] list.
  ...
  /// and the entire [MainTabBar] is scrollable. Otherwise each tab gets an equal
  ...
  /// the indicator with the tab's text for [MainTab] widgets and all but the
  /// shortest [MainTab.text] values.
  ///
  ...
  /// is [MainTabBarIndicatorSize.tab] the tab's bounds are as wide as the space
  /// occupied by the tab in the tab bar. If [indicatorSize] is
  /// [MainTabBarIndicatorSize.label], then the tab's bounds are only as wide as
  ...
  /// tab's overall bounds if [indicatorSize] is [MainTabBarIndicatorSize.tab]
  /// (the default) or relative to the bounds of the tab's widget if
  /// [indicatorSize] is [MainTabBarIndicatorSize.label].
  ///
  ...
  /// An optional callback that's called when the [MainTabBar] is tapped.
  ///
...
class _MainTabBarState extends State<MainTabBar> {
  ...
  _MainIndicatorPainter _indicatorPainter;
  int _currentIndex;
...
//      ),
  ...
  void _handleTap(int index) {
    ...
    _controller.animateTo(index,duration:Duration(seconds: 3),curve: Curves.easeInCirc);
  ...
  Widget build(BuildContext context) {
    ...
    if (_controller != null) {
      ...
      if (_controller.indexIsChanging) {
        ...
        final Animation<double> animation = _MainChangeAnimation(_controller);
        ...
        final Animation<double> centerAnimation =
            _MainDragAnimation(_controller, tabIndex);
        ...
        if (_currentIndex > 0) {
          ...
          final Animation<double> previousAnimation =
              ReverseAnimation(_MainDragAnimation(_controller, tabIndex));
          wrappedTabs[tabIndex] =
...
//          }),
    ...
    if (widget.isScrollable) {
      _scrollController ??= _MainTabBarScrollController(this);
...
/// selected tab.
///
/// This widget is typically used in conjunction with a [MainTabBar].
///
/// If a [TabController] is not provided, then there must be a [DefaultTabController]
/// ancestor.
///
/// The tab controller's [TabController.length] must equal the length of the
/// [children] list and the length of the [MainTabBar.tabs] list.
///
/// To see a sample implementation, visit the [TabController] documentation.
class MainTabBarView extends StatefulWidget {
  ...
  /// One widget per tab.
  ///
  /// Its length must match the length of the [MainTabBar.tabs]
...
class _MainTabBarViewState extends State<MainTabBarView> {
  ...
  @override
  void didUpdateWidget(MainTabBarView oldWidget) {
    super.didUpdateWidget(oldWidget);
...
///
/// Used by [MainTabPageSelector] to indicate the selected page.
class MainTabPageSelectorIndicator extends StatelessWidget {
  /// Creates an indicator used by [MainTabPageSelector].
  ///
  /// The [backgroundColor], [borderColor], and [size] parameters must not be null.
  const MainTabPageSelectorIndicator({
    Key key,
...
/// If a [TabController] is not provided, then there must be a
/// [DefaultTabController] ancestor.
class MainTabPageSelector extends StatelessWidget {
  /// Creates a compact widget that indicates which tab has been selected.
  const MainTabPageSelector({
    Key key,
    this.controller,
  ...
  Widget _buildTabIndicator(
    ...
    return MainTabPageSelectorIndicator(
...
class _InkRippleFactory extends InteractiveInkFeatureFactory {
  _InkRippleFactory(this.onRemoved);
